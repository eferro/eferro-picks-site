...
describe('TalksList Integration', () => {
  ...
  describe('Filter Workflows', () => {
    it('displays all talks when no filters are active', () => {
      ...
      // All talks visible (may appear in both Recently Added and main sections)
      expect(screen.getAllByText('Talk One').length).toBeGreaterThanOrEqual(1);
      expect(screen.getAllByText('Talk Two').length).toBeGreaterThanOrEqual(1);
    });
  ...
  describe('Conference Filter Workflows', () => {
    ...
    it('displays conference buttons on talk cards', () => {
      ...
      // Real TalkCard component renders conference button (may appear in multiple sections)
      expect(screen.getAllByRole('button', { name: /filter by conference ndc london/i }).length).toBeGreaterThanOrEqual(1);
    });
  });
  ...
  describe('Topic Filter Workflows', () => {
    it('displays topic buttons on talk cards', () => {
      ...
      // Real TalkCard component renders topic buttons (may appear in multiple sections)
      expect(screen.getAllByRole('button', { name: /filter by topic react/i }).length).toBeGreaterThanOrEqual(1);
      expect(screen.getAllByRole('button', { name: /filter by topic hooks/i }).length).toBeGreaterThanOrEqual(1);
    });
  });
