...
- User workflows (filtering, navigation, form submission)
- Data transformation pipelines (useTalks with real processTalks)

**Use unit tests for:**
- Pure functions (TalksFilter class, utility functions)
...
it('user can filter talks by rating', () => {
  // Mock only the data boundary (fetch/API)
  const talks = [
  ...
  // Verify user sees filtered results - real TalksFilter runs
...
**Benefits:**
- Tests survive refactoring (no component mocks)
- Catches integration bugs (real child components)
- Higher confidence (tests actual behavior)
- Less maintenance (fewer mocks to update)

**Key principle:** Only mock external boundaries (API, localStorage), never internal code.
...
```typescript
// ✅ GOOD: Integration test for TalksList
renderIntegration(<TalksList />, {
  initialParams: new URLSearchParams('rating=5&hasNotes=true')
});

// Real TalkSection renders
// Real TalksFilter runs
// Real hasMeaningfulNotes runs
// Verify user sees filtered results
expect(screen.getByText('5-star talk with notes')).toBeInTheDocument();
```

**Example - Unit Test:**
```typescript
// ✅ GOOD: Unit test for TalksFilter
const filter = new TalksFilter({ rating: 5, hasNotes: true });
